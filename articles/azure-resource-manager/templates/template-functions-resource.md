...
You can't use the reference function in the outputs of a [nested template](linked-templates.md#nested-template) to return a resource you've deployed in the nested template with `expressionEvaluationOptions` property set to `outer`. Instead, use a [linked template](linked-templates.md#linked-template).
